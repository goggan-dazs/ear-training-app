  ...
  return (
    <div className="pageContainer">
      ...
      <div className='playButtons'>
        <Button variant="outlined" className="the-one-button" onClick={playSound}>play me</Button>
      </div> 
          ...
          {chordsKeys.map(chord => {
            const correctedName = chord.split('_')
             const corrected = correctedName[0][0].toUpperCase() + correctedName[0].substring(1) + " " + (correctedName.length === 2? correctedName[1][0].toUpperCase() + correctedName[1].substring(1) : "")
              ...
              return <Button variant="outlined" className='button' 
                  key={chord}  
      ...
      <div className='playAgainButton'>
        <Button variant="outlined" className='button' onClick={playSoundAgain}>One more time pls</Button>
      </div>
    </div>
  );
}
